...
export function parseCidIpfsIo(link: string): string {
  const result = "https://ipfs.io/ipfs/" + parseCid(link);
  return result;
}

export function parseCidPinata(link: string): string {
  const result = "https://gateway.pinata.cloud/ipfs/" + parseCid(link);
  return result;
}
